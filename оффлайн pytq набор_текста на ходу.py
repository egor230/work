...
def is_speech(audio_data, threshold=0.01, min_duration=3.5, sample_rate=44100):
 """
 Определяет, есть ли звук в аудиопотоке.
 ...
 if avg_amplitude > threshold: # Проверяем, превышает ли средняя амплитуда порог
  return True
 else:
  return False
...
class MyThread(QtCore.QThread):  # Определение класса потока
 ...
 def __init__(self, parent=None):  # Конструктор класса потока
  ...
  self.duration = 8.5  # Начальная длительность записи
