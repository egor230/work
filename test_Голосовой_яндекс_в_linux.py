...
mic_on = True
tray = None   # будет создано в start_tray() # len([message.text.strip() for message in driver.find_elements(By.CLASS_NAME, 'message-bubble_container_from-user')])
previous_message = ""
...
mode_button = WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.CSS_SELECTOR, 'button.model-switcher__button')))
mode_button.click()
...
counts =0
def update_label(soup, url,mic_button):
  global counts
  try:
   message, counts1 = get_latest_message(driver, counts)  # print(message)
   ...
   aria_label = mic_button.get_attribute('aria-label')  # Проверяем, активен ли микрофон   # print( counts1)   # print(filter_elem)
   filter_elem = driver.find_element(By.CSS_SELECTOR, "div.yamb-oknyx-lottie.svelte-8z6ghg").get_attribute("data-testid")  # .get_attribute("data-testid")
   # oknyx_lottie = soup.find('div', class_='yamb-oknyx-lottie')
   # if oknyx_lottie:
    # active_svg = oknyx_lottie.find('svg', class_=lambda x: x != 'animation-hidden')
    # if active_svg:# and 'стоп' in aria_label:
  # else:
  #  root.withdraw()
   if (counts1 > counts and len(message) != 0 and not any(phrase in message for phrase in excluded_phrases)):
    ...
    time.sleep(1.2)
    counts = counts1  # break    # time.sleep(1.4)
   ...
   print(ex1)
   pass
  root.after(150, lambda: update_label(soup, url, mic_button))
...
update_label(soup, url,mic_button)
root.mainloop()
