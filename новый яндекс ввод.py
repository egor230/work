    ...
    while self._running:
      try:
        if not self.mic:
          ...
          if "animation_listening" in classes and "стоп" in aria_label.lower():
            time.sleep(1)
            if message:
  ...
  def run(self):
   ...
   while self._running:
     try:
       ...
       if "animation_collapsedOut" in classes or "animation_thinking" in classes :
         time.sleep(1)
       ...
       if self.mic and "suspended" in filter_elem and "слушать" in aria_label.lower() and "animation_standby" in classes:
         time.sleep(2)
         self.button.click()
...
# standalone_elem = self.driver.find_element(By.CSS_SELECTOR, ".Standalone")
# scrollable_elem = self.driver.find_element(By.CSS_SELECTOR, ".AliceChat-Scrollable")
# chat_elem = self.driver.find_element(By.CSS_SELECTOR, ".AliceChat")
# sc_cls = scrollable_elem.get_attribute("class") or ""
# "AliceChat-Scrollable_empty" not in sc_cls
# st_cls = standalone_elem.get_attribute("class") or ""
# "Standalone_start" not in st_cls
# ch_cls = chat_elem.get_attribute("class") or ""
# "AliceChat_empty" not in ch_cls
# print(standalone_elem.get_attribute("class"))
