...
def search_image():
  try:
    ...
    try:
        with Image.open(image_path1) as im:
            img1_w, img1_h = im.size
    except:
        img1_w, img1_h = img_w, img_h

    if region1[2] < img1_w or region1[3] < img1_h:
        region1 = (0, 0, pyautogui.size().width, pyautogui.size().height)

    search_regions = get_nemo_search_regions(image_path, image_path1)

    if search_regions:
        region, region1 = search_regions

    loc = pyautogui.locateOnScreen(
        image_path, confidence=0.20,
        region=region,  grayscale=True
    )

    loc1 = pyautogui.locateOnScreen(
        image_path1,  confidence=0.2,
