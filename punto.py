   ...
   while True:
    ...
    for line in lines:
     ...
     if re.search(pattern, dir_process_name) and process_id == int(line.split()[1]):
      ...
      if ".exe" in file_path_lower and "winword.exe" not in file_path_lower:
       self.disabled = True
       self.current_word = ""
       self.key_sequence = ""
       self.suggestions = []
 ...
 def _on_press(self, key): # Обрабатывает нажатия клавиш
  ...
  if key == keyboard.Key.space:
   if self.abbrev_res:
    ...
    if self.suggestions:
     self._replace_word_async(self.suggestions[0], extra_backspace=0)
     return
    else:
     self.root.after(0, self._do_hide_all)
   return
  control_keys = { # Набор управляющих клавиш
