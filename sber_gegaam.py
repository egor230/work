import hashlib, logging, math, os, urllib.request, warnings
from abc import ABC, abstractmethod
from pathlib import Path
from subprocess import CalledProcessError, run
from typing import Dict, List, Optional, Tuple, Union
import hydra, numpy as np, omegaconf, onnxruntime as rt, torch, torch.nn.functional as F, torchaudio, torchaudio.functional as taF
...
class FeatureExtractor(nn.Module):
    def __init__(self, sample_rate: int, features: int, hop_length=None, win_length=None, n_fft=None, center=None, **kwargs):
        ...
        self.featurizer = nn.Sequential(torchaudio.transforms.MelSpectrogram(sample_rate=sample_rate, n_mels=features, win_length=self.win_length, hop_length=self.hop_length, n_fft=self.n_fft, center=self.center), SpecScaler())
...
class StridingSubsampling(nn.Module):
    def __init__(self, subsampling: str, kernel_size: int, subsampling_factor: int, feat_in: int, feat_out: int, conv_channels: int):
        super().__init__()
        ...
        for _ in range(self._sampling_num):
            layers.append(subs_conv_class(in_channels=in_channels, out_channels=conv_channels, kernel_size=self._kernel_size, stride=self._stride, padding=self._padding))
...
class MultiHeadAttention(nn.Module, ABC):
    def __init__(self, n_head: int, n_feat: int, flash_attn=False, torch_sdpa_attn=False):
        ...
        if self.flash_attn and not IMPORT_FLASH:
            raise RuntimeError(f"flash_attn_func was imported with err {IMPORT_FLASH_ERR}. Please install flash_attn or use --no_flash flag. If you have already done this, --force-reinstall flag might be useful")
...
class RotaryPositionMultiHeadAttention(MultiHeadAttention):
    def forward(self, query: Tensor, key: Tensor, value: Tensor, pos_emb: List[Tensor], mask: Optional[Tensor] = None) -> Tensor:
        ...
        q, k, v = self.forward_qkv(query.view(t, b, self.h * self.d_k).transpose(0, 1), key.view(t, b, self.h * self.d_k).transpose(0, 1), value.view(t, b, self.h * self.d_k).transpose(0, 1))
        if self.flash_attn:
            if mask is None:
...
class ConformerConvolution(nn.Module):
    def __init__(self, d_model: int, kernel_size: int, norm_type: str):
        ...
        self.depthwise_conv = nn.Conv1d(in_channels=d_model, out_channels=d_model, kernel_size=kernel_size, padding=(kernel_size - 1) // 2, groups=d_model, bias=True)
...
class ConformerLayer(nn.Module):
    def __init__(self, d_model: int, d_ff: int, self_attention_model: str, n_heads: int = 16, conv_norm_type: str = "batch_norm", conv_kernel_size: int = 31, flash_attn: bool = False):
        super().__init__()
        self.fc_factor = 0.5
        ...
        if self_attention_model == "rotary":
            self.self_attn: nn.Module = RotaryPositionMultiHeadAttention(n_head=n_heads, n_feat=d_model, flash_attn=flash_attn, torch_sdpa_attn=not flash_attn)
        else:
    ...
    def forward(self, x: Tensor, pos_emb: Union[Tensor, List[Tensor]], att_mask: Optional[Tensor] = None, pad_mask: Optional[Tensor] = None) -> Tensor:
        residual = x
...
class ConformerEncoder(nn.Module):
    def __init__(self, feat_in: int = 64, n_layers: int = 16, d_model: int = 768, subsampling_factor: int = 4, ff_expansion_factor: int = 4, self_attention_model: str = "rotary", n_heads: int = 16, pos_emb_max_len: int = 5000, conv_kernel_size: int = 31, flash_attn: bool = False, subsampling=None, subs_kernel_size=None, conv_norm_type=None, **kwargs):
        super().__init__()
        ...
        self.pre_encode = StridingSubsampling(subsampling=subsampling, kernel_size=subs_kernel_size, subsampling_factor=subsampling_factor, feat_in=feat_in, feat_out=d_model, conv_channels=d_model)
        ...
        for _ in range(n_layers):
            layer = ConformerLayer(d_model=d_model, d_ff=d_model * ff_expansion_factor, self_attention_model=self_attention_model, n_heads=n_heads, conv_norm_type=conv_norm_type, conv_kernel_size=conv_kernel_size, flash_attn=flash_attn)
            self.layers.append(layer)

    def input_example(self, batch_size: int = 1, seqlen: int = 200) -> Tuple[Tensor, Tensor]:
        device = next(self.parameters()).device
    ...
    def dynamic_axes(self) -> Dict[str, Dict[int, str]]:
        return {"audio_signal": {0: "batch_size", 2: "seq_len"}, "length": {0: "batch_size"}, "encoded": {0: "batch_size", 1: "seq_len"}, "encoded_len": {0: "batch_size"}}
...
def segment_audio_file(wav_input: Union[np.ndarray, Tensor], sr: int, max_duration: float = 22.0, min_duration: float = 15.0, strict_limit_duration: float = 30.0, new_chunk_threshold: float = 0.2, device: torch.device = torch.device("cpu")) -> Tuple[List[torch.Tensor], List[Tuple[float, float]]]:
...
def infer_onnx(wav_input: Union[str, np.ndarray, Tensor], model_cfg: omegaconf.DictConfig, sessions: List[rt.InferenceSession], preprocessor: Optional[FeatureExtractor] = None, tokenizer: Optional[Tokenizer] = None, sample_rate: int = 16000) -> Union[str, np.ndarray]:
    ...
    enc_sess = sessions[0]
    enc_inputs = {node.name: data for (node, data) in zip(enc_sess.get_inputs(), [input_signal.astype(DTYPE), [input_signal.shape[-1]]])}
    ...
    if "ctc" in model_name:
        ...
        pred_states = [np.zeros(shape=(1, 1, model_cfg.head.decoder.pred_hidden), dtype=DTYPE), np.zeros(shape=(1, 1, model_cfg.head.decoder.pred_hidden), dtype=DTYPE)]
        pred_sess, joint_sess = sessions[1:]
        for j in range(enc_features.shape[-1]):
            emitted_letters = 0
            while emitted_letters < MAX_LETTERS_PER_FRAME:
                pred_inputs = {node.name: data for (node, data) in zip(pred_sess.get_inputs(), [np.array([[prev_token]])] + pred_states)}
                pred_outputs = pred_sess.run([node.name for node in pred_sess.get_outputs()], pred_inputs)
                joint_inputs = {node.name: data for node, data in zip(joint_sess.get_inputs(), [enc_features[:, :, [j]], pred_outputs[0].swapaxes(1, 2)])}
...
def load_onnx(onnx_dir: str, model_version: str, provider: Optional[str] = None) -> Tuple[List[rt.InferenceSession], Union[omegaconf.DictConfig, omegaconf.ListConfig]]:
...
def onnx_converter(model_name: str, module: torch.nn.Module, out_dir: str, inputs: Optional[Tuple[Tensor, ...]] = None, input_names: Optional[List[str]] = None, output_names: Optional[List[str]] = None, dynamic_axes: Optional[Union[Dict[str, List[int]], Dict[str, Dict[int, str]]]] = None, opset_version: int = 17):
    if inputs is None:
    ...
    with warnings.catch_warnings(), torch.no_grad():
        ...
        torch.onnx.export(module.to(torch.float32), inputs, out_path, input_names=input_names, output_names=output_names, dynamic_axes=dynamic_axes, opset_version=opset_version)
...
def apply_masked_flash_attn(q: Tensor, k: Tensor, v: Tensor, mask: Tensor, h: int, d_k: int) -> Tensor:
    ...
    output_unpad = flash_attn_varlen_func(q_unpad, k_unpad, v_unpad, cu_seqlens_q, cu_seqlens_q, max_seqlen_q, max_seqlen_q)
    scores = pad_input(rearrange(output_unpad, "nnz h d -> nnz (h d)"), indices_q, b, t)
    return scores
...
class AudioDataset(torch.utils.data.Dataset):
    def __init__(self, lst: List[Union[str, np.ndarray, torch.Tensor]]):
        ...
        assert isinstance(lst[0], (str, np.ndarray, torch.Tensor)), f"Unexpected dtype: {type(lst[0])}"
        self.lst = lst
...
class GigaAM(nn.Module):
    ...
    def embed_audio(self, wav_input: Union[str, np.ndarray, Tensor], sample_rate: int = SAMPLE_RATE) -> Tuple[Tensor, Tensor]:
    ...
    def _to_onnx(self, dir_path: str = ".") -> None:
        onnx_converter(model_name=f"{self.cfg.model_name}_encoder", out_dir=dir_path, module=self.encoder, dynamic_axes=self.encoder.dynamic_axes())
...
class GigaAMASR(GigaAM):
    ...
    def transcribe(self, wav_input: Union[str, np.ndarray, Tensor], sample_rate: int = SAMPLE_RATE, **kwargs) -> Union[str, List[Dict[str, Union[str, Tuple[float, float]]]]]:
        ...
        if audio_length <= LONGFORM_THRESHOLD:
            ...
            segments, boundaries = segment_audio_file(audio_data, SAMPLE_RATE, device=self._device, **kwargs)
    ...
    def _to_onnx(self, dir_path: str = ".") -> None:
        if "ctc" in self.cfg.model_name:
            ...
            try:
                onnx_converter(model_name=self.cfg.model_name, out_dir=dir_path, module=self, inputs=self.encoder.input_example(), input_names=["features", "feature_lengths"], output_names=["log_probs"], dynamic_axes={"features": {0: "batch_size", 2: "seq_len"}, "feature_lengths": {0: "batch_size"}, "log_probs": {0: "batch_size", 1: "seq_len"}})
            finally:
                self.forward = saved_forward  # type: ignore[assignment, method-assign]
        else:
            super()._to_onnx(dir_path)  # export encoder
            onnx_converter(model_name=f"{self.cfg.model_name}_decoder", out_dir=dir_path, module=self.head.decoder)
            onnx_converter(model_name=f"{self.cfg.model_name}_joint", out_dir=dir_path, module=self.head.joint)
...
class GigaAMEmo(GigaAM):
    ...
    def _to_onnx(self, dir_path: str = ".") -> None:
        ...
        self.forward = self.forward_for_export  # type: ignore[assignment, method-assign]
        try:
            onnx_converter(model_name=self.cfg.model_name, out_dir=dir_path, module=self, inputs=self.encoder.input_example(), input_names=["features", "feature_lengths"], output_names=["probs"], dynamic_axes={"features": {0: "batch_size", 2: "seq_len"}, "feature_lengths": {0: "batch_size"}, "probs": {0: "batch_size", 1: "seq_len"}})
        finally:
            self.forward = saved_forward  # type: ignore[assignment, method-assign]

__all__ = ["GigaAM", "GigaAMASR", "GigaAMEmo", "load_audio", "format_time", "load_model"]
...
def _download_file(file_url: str, file_path: str, force: bool = False) -> str:
    ...
    with urllib.request.urlopen(file_url) as source, open(file_path, "wb") as output:
        with tqdm(total=int(source.info().get("Content-Length", 0)), ncols=80, unit="iB", unit_scale=True, unit_divisor=1024) as loop:
            while True:
...
def load_model(model_name: str, fp16_encoder: bool = True, use_flash: Optional[bool] = False, device: Optional[Union[str, torch.device]] = None, download_root: Optional[str] = None, force_download: bool = False) -> Union[GigaAM, GigaAMEmo, GigaAMASR]:
    ...
    if os.path.exists(model_path):
        ...
        if expected_hash and actual_hash != expected_hash:
            if force_download:
                ...
                raise RuntimeError(f"Model checksum failed for {model_name}. Expected {expected_hash}, got {actual_hash}. Please delete {model_path} and reload the model, or use force_download=True to re-download automatically.")
    if not os.path.exists(model_path):
