...
def get_word():
  ...
  # Отключить сообщения ALSA
  # sys.stderr = open(os.devnull, 'w')
  # Устанавливаем переменные среды для работы PulseAudio
  ...
  try:
   with mic as source:
    r.adjust_for_ambient_noise(source)  # Настраиваем микрофон на шум
    audio = r.listen(source)
    text = r.recognize_google(audio, language="ru-RU")
    return text
  except:
    pass
def prease_on_key(driver, d): # print(d)
 pres12= work_key()
 timestamp = time.time()
 while 1:
  try:
   text = get_word() #driver.find_element("id", "speech-text").text
   text = str(text)
...
def start_voice():
  try:
    ...
    t1 = threading.Thread(target=prease_on_key, args=(driver, d,))
    t1.start()
    while 1:
      time.sleep(1)
      pass
