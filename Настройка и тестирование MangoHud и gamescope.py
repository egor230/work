...
# --- Переключаемые параметры MangoHud ---
MH_TOGGLES = [
    ("fps", "FPS"),
    ...
    ("engine_version", "Версия движка / DXVK (engine_version)"),
    ...
    ("device_battery_icon", "Иконка батареи устройства (device_battery_icon)"),
    ...
    ("horizontal_stretch", "Растяжение по горизонтали (horizontal_stretch)"),
    ("hud_compact", "Компактный HUD (hud_compact)"),
    ("hud_no_margin", "Без отступов HUD (hud_no_margin)"),
    ("no_display", "Скрыть HUD (no_display)"),
    ("no_small_font", "Не использовать мелкий шрифт (no_small_font)"),
    ("temp_fahrenheit", "Температура в Фаренгейтах (temp_fahrenheit)"),
    ("fps_metrics", "Метрики FPS avg (fps_metrics)"),
    ("fcat", "FCAT (fcat)"),
]


# Поля MangoHud, которые не являются обычными чекбоксами
MH_FIXED_FIELDS = [
    "enabled",
    "position",
    "fps_only",
...
# Поля GameScope для синхронизации виджетов
GS_FIELDS = [
    "enabled",
    "width",
    "height",
...
# Включаем только самые базовые метрики
for key, _ in MH_TOGGLES:
    DEFAULT_MH[key] = key in (
        "fps",
        "gpu_temp",
        "vram",
        "ram",
...
# Настройки GameScope по умолчанию
DEFAULT_GS = {
    "enabled": False,
    "width": 1280,
    "height": 720,
    ...
    "prefer_vk_device": "",
    "hdr_enabled": False,
    "hdr_itm_enable": False,
    "sdr_gamut_wideness": 0.5,
    "hdr_sdr_content_nits": 400,
    "hdr_itm_sdr_nits": 0,
    "hdr_itm_target_nits": 0,
    "force_composition": False,
    "hdr_force_support": False,
    "hdr_force_output": False,
    "hdr_force_heatmap": False,
...
# Очистка ключей словаря от пробелов
def _clean_keys(d):
...
class GameScopeMangoHudApp(QMainWindow):
    def __init__(self):
        ...
        # Путь к JSON рядом со скриптом
        script_dir = os.path.dirname(os.path.abspath(__file__))
        self.config_path = os.path.join(script_dir, CONFIG_FILE)

        # Единая модель данных
        self._data = {
            "mangohud": dict(DEFAULT_MH),
            "gamescope": dict(DEFAULT_GS),
        }

        # Флаг защиты от рекурсивной синхронизации
        self._syncing = False

        # Текущий процесс предпросмотра
        self._preview_process = None
        self._preview_mode = None

        # Таймер отложенного перезапуска предпросмотра
        self._restart_timer = QTimer(self)
        self._restart_timer.setSingleShot(True)
        self._restart_timer.setInterval(300)
        self._restart_timer.timeout.connect(self._do_restart_preview)

        # Сохранённая геометрия окна vkcube
        self._cube_geom = None
        ...
        # Загрузка настроек и синхронизация интерфейса
        self._read_config()
        self._sync_all_widgets()

        # Восстановление геометрии окна приложения
    ...
    def _build_tab(self, mode):
        ...
        # Кнопка сохранения только на совместной вкладке
        if mode == "both":
            sr = QHBoxLayout()
            sr.addStretch()
        ...
        # Сохраняем ссылки на элементы вкладки
    ...
    def _build_mh_ui(self, parent_layout, mode):
        ...
        # Лимит FPS через MangoHud
        fps_row = QHBoxLayout()

        spn_fps_lim = QSpinBox()
        spn_fps_lim.setRange(0, 999)
        spn_fps_lim.setValue(0)
        ...
        for i, (key, label) in enumerate(MH_TOGGLES):
            ...
            # Подсказка из текста в скобках
            if "(" in label:
                tip = label.split("(")[-1].rstrip(")")
            else:
                tip = label

            chk.setToolTip(f"Показывать {tip}")
        ...
        # Все чекбоксы обновляют предпросмотр
    ...
    def _build_gs_ui(self, parent_layout, mode):
        ...
        spn_ow = QSpinBox()
        spn_ow.setRange(0, 7680)
        spn_ow.setValue(0)
        spn_ow.setSpecialValueText("Авто")
        spn_ow.setToolTip("Ширина выходного разрешения (-W)")

        spn_oh = QSpinBox()
        spn_oh.setRange(0, 4320)
        spn_oh.setValue(0)
        spn_oh.setSpecialValueText("Авто")
        spn_oh.setToolTip("Высота выходного разрешения (-H)")

        r2.addWidget(QLabel("Ш:"))
        r2.addWidget(spn_ow)
        r2.addWidget(QLabel("В:"))
        r2.addWidget(spn_oh)

        dl.addRow("Выходное разрешение (-W -H):", r2)
        target["out_width"] = spn_ow
        target["out_height"] = spn_oh

        # Частота обновления
        spn_rr = QSpinBox()
        spn_rr.setRange(0, 480)
        spn_rr.setValue(0)
        spn_rr.setSpecialValueText("По умолч.")
        spn_rr.setToolTip("Частота обновления (-r)")
        ...
        chk_bl = QCheckBox("Без рамки (-b)")
        chk_bl.setChecked(True)
        ...
        nr.addWidget(chk_bl)
        nr.addWidget(chk_fs)
        nr.addWidget(chk_ffs)

        dl.addRow("Режим окна:", nr)

        target["borderless"] = chk_bl
        target["fullscreen"] = chk_fs
        ...
        dsp_msf.setRange(0.0, 100.0)
        dsp_msf.setValue(0.0)
        dsp_msf.setSpecialValueText("Выкл")
        dsp_msf.setSingleStep(0.5)
        ...
        # Лимит FPS
        spn_fl = QSpinBox()
        spn_fl.setRange(0, 999)
        spn_fl.setValue(0)
        spn_fl.setSpecialValueText("Выкл")
        spn_fl.setToolTip("Лимит FPS через --framerate-limit")
        ...
        chk_gk = QCheckBox("Захват клавиатуры (-g)")
        chk_gk.setToolTip("Захватить ввод с клавиатуры")
        ...
        adv_chks.addWidget(chk_gk)
        adv_chks.addWidget(chk_fgc)
        adv_chks.addWidget(chk_ew)
        adv_chks.addWidget(chk_rt)
        ...
        hdr_nits1 = QSpinBox()
        hdr_nits1.setRange(0, 10000)
        hdr_nits1.setValue(400)
        ...
        # ITM SDR ниты
        hdr_nits2 = QSpinBox()
        hdr_nits2.setRange(0, 1000)
        hdr_nits2.setValue(0)
        ...
        # Целевая яркость ITM
        hdr_nits3 = QSpinBox()
        hdr_nits3.setRange(0, 10000)
        hdr_nits3.setValue(0)
        ...
        dsp_sgw.setRange(0.0, 1.0)
        dsp_sgw.setValue(0.5)
        dsp_sgw.setSingleStep(0.1)
        dsp_sgw.setDecimals(1)
        ...
        hdr_debug_row.addWidget(chk_fc)
        hdr_debug_row.addWidget(chk_hfs)
        hdr_debug_row.addWidget(chk_hfo)
        hdr_debug_row.addWidget(chk_hfh)
        ...
        # Добавляем группы в форму
        form.addRow(ag)
        form.addRow(hg)

        parent_layout.addLayout(form)

        # Подключение сигналов GameScope
    ...
    def _on_changed(self, section):
        ...
        # Определяем активный набор виджетов
        if idx == 0:
            mh_w = self._mh_w1
        elif idx == 1:
            gs_w = self._gs_w2
        ...
        if section == "mh" and mh_w is not None:
            ...
            for key in MH_FIXED_FIELDS:
                w = mh_w.get(key)
                if w:
                    d[key] = _get_widget_value(w)

            toggles = mh_w.get("toggles", {})
            for key, chk in toggles.items():
                d[key] = chk.isChecked()

        # Обновляем данные GameScope
        if section == "gs" and gs_w is not None:
            for key in GS_FIELDS:
                w = gs_w.get(key)
                if w:
                    self._data["gamescope"][key] = _get_widget_value(w)

        # Автосохранение
        self._save_config()

        # Если предпросмотр запущен, перезапускаем с задержкой
        if self._preview_process is not None:
            self._restart_timer.start()

        # Обновляем текст команд
        self._update_previews()

    # Синхронизация всех виджетов
    def _sync_all_widgets(self):
        ...
        for target in [self._mh_w1, self._mh_w3]:
            ...
            for key in MH_FIXED_FIELDS:
                w = target.get(key)
                if w:
                    _set_widget_value(w, d.get(key, DEFAULT_MH.get(key)))
        ...
        for target in [self._gs_w2, self._gs_w3]:
            ...
            for key in GS_FIELDS:
                w = target.get(key)
                if w:
                    _set_widget_value(w, d.get(key, DEFAULT_GS.get(key)))
    ...
    # Сборка строки MANGOHUD_CONFIG
    def _mh_config_string(self) -> str:
        d = self._data["mangohud"]

        # Позиция всегда первой
    ...
    # Сборка переменных окружения MangoHud
    ...
    def _build_gs_args(self):
        ...
        # Выходное разрешение
        ow = d.get("out_width", 0)
        oh = d.get("out_height", 0)

        if ow > 0:
            args.extend(["-W", str(ow)])

        if oh > 0:
            args.extend(["-H", str(oh)])

        # Частота обновления
        rr = d.get("refresh", 0)
        if rr > 0:
            args.extend(["-r", str(rr)])

        # Режим окна
        if d.get("borderless", True):
            args.append("-b")

        if d.get("fullscreen", False):
            args.append("-f")

        if d.get("force_fullscreen", False):
            args.append("--force-windows-fullscreen")
        ...
        if msf > 0.0:
            args.extend(["-m", f"{msf:.1f}"])

        # Лимит FPS
        fl = d.get("framerate_limit", 0)
        if fl > 0:
            args.extend(["--framerate-limit", str(fl)])
        ...
        if bv:
            args.extend(["--backend", bv])

        # Захват клавиатуры
        if d.get("grab_keyboard"):
            args.append("-g")

        # Принудительный захват курсора
        if d.get("force_grab_cursor"):
            args.append("--force-grab-cursor")

        # Wayland
        if d.get("expose_wayland"):
            args.append("--expose-wayland")

        # Realtime scheduling
        if d.get("realtime_scheduling"):
            args.append("--rt")
        ...
        if vk_dev:
            args.extend(["--prefer-vk-device", vk_dev])

        # HDR
        if d.get("hdr_enabled"):
            args.append("--hdr-enabled")

        if d.get("hdr_itm_enable"):
            args.append("--hdr-itm-enabled")
        ...
        # Отладочные HDR-флаги
        if d.get("force_composition"):
            args.append("--force-composition")

        if d.get("hdr_force_support"):
            args.append("--hdr-debug-force-support")

        if d.get("hdr_force_output"):
            args.append("--hdr-debug-force-output")

        if d.get("hdr_force_heatmap"):
            args.append("--hdr-debug-heatmap")

        return args

    # Сборка итоговой команды
    def _build_command(self, mode, target):
        ...
        # Сначала GameScope, если включён
        if gs_args:
        ...
        if env_str:
            return f"{env_str} {cmd}"

        return cmd

    # Сохранить позицию окна vkcube
                ...
                if output:
                    ...
                    subprocess.run(
                        [
                            ...
                            "windowmove",
                            wid,
                            str(self._cube_geom["x"]),
                            str(self._cube_geom["y"]),
                        ],
                        timeout=2,
                        check=True,
                    )
                    ...
                    print(
                        "Восстановлена позиция vkcube: "
                        f"x={self._cube_geom['x']}, y={self._cube_geom['y']}"
                    )
                    return
    ...
    def _start_preview(self, mode):
        ...
        if not cmd:
            QMessageBox.information(
                self,
                "Предпросмотр",
                "Включите MangoHud или GameScope.",
            )
            return
    ...
    def _do_restart_preview(self):
        ...
        # Сохраняем позицию старого окна
    ...
    # Остановка предпросмотра для конкретной вкладки
    ...
    # После завершения предпросмотра
    ...
    # Сохранить геометрию окна приложения
    ...
    # Восстановить геометрию окна приложения
    ...
    def _read_config(self):
        try:
            ...
            # Поддержка старых ключей с пробелами
            raw = _clean_keys(raw)

            # Чтение секций
            for section, defaults in [
                ("mangohud", DEFAULT_MH),
                ("gamescope", DEFAULT_GS),
            ]:
                saved = _clean_keys(raw.get(section, {}))

                for k, default_val in defaults.items():
                    if k in saved:
                        self._data[section][k] = saved[k]

            # Отдельно перечитываем переключатели MangoHud
            saved_mh = _clean_keys(raw.get("mangohud", {}))
            for key, _ in MH_TOGGLES:
                if key in saved_mh:
                    self._data["mangohud"][key] = saved_mh[key]

            # Геометрия окна
            if "_window_geometry" in raw:
                self._data["_window_geometry"] = raw["_window_geometry"]
    ...
    def _save_config(self):
        ...
        data = {
            "mangohud": dict(self._data["mangohud"]),
            "gamescope": dict(self._data["gamescope"]),
        }

        # Сохраняем геометрию окна
        if "_window_geometry" in self._data:
            data["_window_geometry"] = self._data["_window_geometry"]
...
def main():
    ...
    # Стиль Fusion, чтобы избежать проблем с GTK-темами
