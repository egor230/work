...
# --- Конфигурация модели ---
# Путь к ПАПКЕ, куда был перемещен GGML-файл и переименован в model.bin
MODEL_FILE = "/mnt/807EB5FA7EB5E954/soft/Virtual_machine/linux must have/python_linux/work/cache/podlodka_model/ggml-large-v3-podlodka-q8_0.bin"
warnings.filterwarnings("ignore", category=DeprecationWarning)

# Настройка директории кэша (остается, если нужна)
...
if not os.path.exists(MODEL_FILE):
  print(f"Ошибка: Папка модели не найдена по пути: {MODEL_FILE}.")
  sys.exit(1)

# Проверяем наличие файла 'model.bin' внутри папки (согласно fix-у)
# if not os.path.exists(os.path.join(MODEL_FILE, "model.bin")):
#   print(f"Ошибка: Файл model.bin не найден внутри папки {MODEL_FILE}. Убедитесь, что GGML-файл переименован.")
#   sys.exit(1)
...
# Если процесс найден — пытаемся убить все экземпляры
if result.returncode == 0 and result.stdout.strip():
  pids = result.stdout.strip().split()
  for pid in pids:
    try:
      # Меняем kill -9 на более мягкий kill, игнорируя ошибки, если процесс уже умер
      subprocess.run(["kill", "-9", pid], check=False)
      print(f"Отправлен сигнал завершения процессу {script_name} с PID {pid}")
    except Exception as e:
      print(f"Не удалось отправить сигнал PID {pid}: {e}")
...
# Запуск вспомогательного скрипта
threading.Thread(target=run_script, daemon=True).start()

# --- Загрузка модели ---
try:  # Загрузка модели: теперь передаем путь к папке
  model = WhisperModel(MODEL_FILE, device="cpu", compute_type="int8_float32")
