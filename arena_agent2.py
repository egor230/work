...
AGENT_FS_INSTRUCTION = (
  "[SYSTEM] You are a file-system agent with shell access.\n"
  "Working directory: {workdir}\n\n"
  "TOOLS:\n"
  "- [EXEC]command[/EXEC] — run a bash command\n"
  "- [EDIT]path\nold text\n---\nnew text[/EDIT] — edit file (old text must be unique)\n"
  "- [EXEC]json_repair /path/file.json[/EXEC] — repair broken JSON\n"
  "- [EXEC]validate_json /path/file.json[/EXEC] — validate JSON\n\n"
  "RULES:\n"
  "1. After each [EXEC] or [EDIT], you receive [RESULT]. Analyze it.\n"
  "2. If a command fails (rc!=0), analyze error and try another way.\n"
  "3. After creating/editing a file, VERIFY:\n"
  "   - .py: python3 -m py_compile file.py\n"
  "   - .json: python3 -c 'import json; json.load(open(\"file.json\"))'\n"
  "   - Other: cat, wc, or ls to confirm\n"
  "4. For Python files, use base64 to preserve indentation:\n"
  "   echo '<base64>' | base64 -d > file.py\n"
  "5. Выполняй только необходимые операции; никогда не повторяй успешно выполненную команду.\n"
  "6. When done, state 'DONE: <summary>' with verification evidence.\n"
  "7. Never run: rm -rf /, mkfs, fdisk, dd to block devices.\n\n"
  "User request:\n"
)
AGENT_TOOL_TIMEOUT = 300
AGENT_MAX_TOOL_ITERS = 6
AGENT_MAX_TOOL_COMMANDS = 12
AGENT_REPEAT_LIMIT = 1


def _normalize_tool_text(text: str) -> str:
  """Normalize a tool payload for stable duplicate detection."""
  return re.sub(r"\s+", " ", text.strip())


def _has_done_marker(text: str) -> bool:
  return bool(re.search(r"(?im)^\s*(?:DONE|ГОТОВО|ГОТОВЫЙ\s+РЕЗУЛЬТАТ)\s*:", text or ""))
...
def _edit_file_tool(spec: str, work_dir: Path) -> tuple[int, str]:
  """Редактирует файл: точная замена old_string → new_string."""
  parts = spec.split("\n---\n", 1)
  if len(parts) != 2:
    return 1, "Формат: [EDIT]путь\nстарое\n---\nновое[/EDIT]"
  header = parts[0].strip()
  body = parts[1]
  sep = body.find("\n")
  if sep == -1:
    return 1, "Формат: old_string (одна строка) --- new_string"
  old_str = body[:sep]
  new_str = body[sep + 1:]
  path = _resolve_safe(header, work_dir)
  if not path.is_file():
    return 1, f"Файл не найден: {path}"
  content = path.read_text(encoding="utf-8")
  if old_str not in content:
    return 1, "old_string не найдена в файле"
  if content.count(old_str) > 1:
    return 1, f"old_string встречается {content.count(old_str)} раз — уточни контекст"
  new_content = content.replace(old_str, new_str, 1)
  if path.suffix == ".py":
    try:
      compile(new_content, str(path), "exec")
    except SyntaxError as e:
      return 1, f"Синтаксическая ошибка после замены, откат: {e}"
  path.write_text(new_content, encoding="utf-8")
  return 0, f"OK: замена в {path.name} ({len(old_str)} → {len(new_str)} байт)"
...
class ArenaWorker(threading.Thread):
  def __init__(self, settings: BrowserSettings, agent_fs: bool = True,
               ...
               tool_timeout: float = 300, max_tool_iters: int = 6,
               max_tool_commands: int = AGENT_MAX_TOOL_COMMANDS,
               task_root: Path = Path("/home/egor")) -> None:
    ...
    self.max_tool_commands = max(1, max_tool_commands)
      ...
      while not self.stop_event.is_set():
        ...
        try:
          ...
          if job.kind == "ask":
            ...
            job.result = self._agent_task(job.payload)
          else:
  ...
  def _safe_task_path(self, directory: str, filename: str, root: Path) -> Path:
    directory_path = _resolve_safe(directory, root)
    directory_path.mkdir(parents=True, exist_ok=True)
    if not directory_path.is_dir():
      raise ValueError(f"не является директорией: {directory_path}")
    if not filename:
      raise ValueError("модель не указала имя файла")
    candidate = _resolve_safe(filename, directory_path)
    if candidate == directory_path or candidate.suffix.lower() != ".py":
      raise ValueError("для agent_task требуется имя Python-файла внутри указанной директории")
    return candidate
  ...
  def _agent_task(self, payload: dict[str, Any]) -> dict[str, Any]:
    """One-shot structured bridge: ask Arena for a complete file, then verify it."""
    request = str(payload["request"]).strip()
    request_id = str(payload.get("request_id") or "").strip()
    if request_id and request_id in self.completed_tasks:
      LOG.info("[AGENT-TASK] returning cached result request_id=%s", request_id)
    ...
    target_dir = _resolve_safe(directory, root)
    target_dir.mkdir(parents=True, exist_ok=True)
    prompt = (
      "[SYSTEM] Ты работаешь как генератор одного файла. Не используй [EXEC] и [EDIT]. "
      "Верни ровно один блок [FILE_WRITE] с полным конечным содержимым файла. "
      "После блока напиши одну строку DONE:. Формат строго такой:\n"
      "[FILE_WRITE]\npath: имя_файла.py\ncontent:\nполный код\n[/FILE_WRITE]\n"
      "DONE: краткое описание и способ проверки.\n"
      f"Целевая директория: {target_dir}\n"
      f"Предпочтительное имя файла: {requested_filename or '(выбери из задания)'}\n"
      "Не создавай несколько файлов и не повторяй блок.\n\n"
      "Задание пользователя:\n" + request
    )
    result = self._ask_raw(prompt, payload.get("model"), float(payload.get("timeout", 300)))
    answer = result["answer"]
    matches = list(_FILE_WRITE_RE.finditer(answer))
    if len(matches) != 1:
      correction = self._ask_raw(
        "Предыдущий ответ нельзя выполнить: нужен ровно один блок [FILE_WRITE] "
        "с полным Python-кодом. Не используй инструменты. Верни только корректный "
        "[FILE_WRITE] и DONE:.", payload.get("model"), 90
      )["answer"]
      matches = list(_FILE_WRITE_RE.finditer(correction))
      answer = correction
    if len(matches) != 1:
      raise RuntimeError("Arena не вернула ровно один структурированный FILE_WRITE-блок")
    relative_name = matches[0].group(1).strip()
    content = matches[0].group(2)
    if requested_filename and Path(relative_name).name != Path(requested_filename).name:
      raise ValueError(f"модель указала файл {relative_name!r}, ожидался {requested_filename!r}")
    target = self._safe_task_path(".", relative_name, target_dir)
    if target.parent != target_dir.resolve():
      raise ValueError("FILE_WRITE может записывать только непосредственно в целевую директорию")
    try:
      compile(content, str(target), "exec")
    except SyntaxError as exc:
      raise RuntimeError(f"модель вернула синтаксически неверный Python: {exc}") from exc
    self._atomic_write_text(target, content)
    try:
      compile(target.read_text(encoding="utf-8"), str(target), "exec")
    except SyntaxError as exc:
      raise RuntimeError(f"проверка записанного файла не пройдена: {exc}") from exc
    result = {
      "ok": True,
      "message": "Всё готово",
      "path": str(target),
  ...
  def _is_looping(self, text: str) -> bool:
    """Детектирует зацикливание: хвост 120 символов повторяется ≥4 раз."""
    TAIL_LEN = 120
    THRESHOLD = 4
  ...
  def _run_tool_loop(self, answer: str, timeout: float, model: str | None) -> dict[str, Any]:
    """Execute tools exactly once per unique payload and force a final response.

    The previous implementation executed whatever command the model repeated.
    This version treats a repeated successful command as a protocol violation:
    it is never executed twice, and the model is asked to close the task.
    """
    loop_deadline = time.time() + min(timeout, 600)
    seen: dict[str, int] = {}
    executed = 0
    complete = False

    def remaining() -> float:
      return max(10.0, loop_deadline - time.time())

    def ask_feedback(text: str) -> str:
      return self._ask_raw(text, None, remaining())["answer"]

    for i in range(self.max_tool_iters):
      if time.time() >= loop_deadline:
        LOG.warning("[TOOL-LOOP] total budget exhausted")
        break
      if self._is_looping(answer):
        LOG.warning("[TOOL-LOOP] repeated reasoning detected at iter=%d", i)
        try:
          answer = ask_feedback(
            "[SYSTEM] Обнаружено зацикливание. Инструменты больше не выполняй. "
            "Верни только итог выполненной задачи, начинающийся с DONE:."
          )
        except Exception:
          pass
        break

      edit_matches = list(_EDIT_RE.finditer(answer))
      if edit_matches:
        payload = edit_matches[0].group(1).strip()
        key = "EDIT:" + _normalize_tool_text(payload)
        seen[key] = seen.get(key, 0) + 1
        if seen[key] > AGENT_REPEAT_LIMIT:
          LOG.warning("[TOOL-LOOP] duplicate edit blocked")
          try:
            answer = ask_feedback(
              "[RESULT]\nПовторный EDIT заблокирован: эта операция уже успешно "
              "выполнена. Не выполняй инструменты. Проверь уже созданный файл "
              "и верни итог с DONE:.\n[/RESULT]"
            )
          except Exception:
            answer = "DONE: Повторный EDIT заблокирован; ранее выполненная операция сохранена."
          if _EDIT_RE.search(answer) or self._extract_tool_commands(answer):
            answer = "DONE: Повторный EDIT заблокирован; ранее выполненная операция сохранена."
          complete = True
          break
        if executed >= self.max_tool_commands:
          break
        rc, out = _edit_file_tool(payload, self.work_dir)
        executed += 1
        if rc == 0:
          seen[key] = AGENT_REPEAT_LIMIT
        else:
          seen[key] = 0
        feedback = f"[RESULT]\n[EDIT] → (rc={rc})\n{out}\n[/RESULT]"
        try:
          answer = ask_feedback(feedback)
        except Exception as exc:
          LOG.warning("[TOOL-LOOP] edit feedback failed: %s", exc)
          break
        continue

      cmds = self._extract_tool_commands(answer)
      if not cmds:
        complete = _has_done_marker(answer) or bool(answer.strip())
        break

      c = cmds[0].strip()
      key = "EXEC:" + _normalize_tool_text(c)
      seen[key] = seen.get(key, 0) + 1
      if seen[key] > AGENT_REPEAT_LIMIT:
        LOG.warning("[TOOL-LOOP] duplicate command blocked: %s", c[:160])
        try:
          answer = ask_feedback(
            "[RESULT]\nПовторная команда заблокирована: она уже была успешно "
            "выполнена и повторно запускаться не будет. Не создавай файл заново. "
            "Выполни только проверку результата, если она действительно нужна, "
            "затем верни финальный ответ с DONE:.\n[/RESULT]"
          )
        except Exception:
          answer = "DONE: Повторная команда заблокирована; ранее выполненная команда уже выполнена."
        if _EDIT_RE.search(answer) or self._extract_tool_commands(answer):
          answer = "DONE: Повторная команда заблокирована; ранее выполненная команда уже выполнена."
        complete = True
        break
      ...
      if executed >= self.max_tool_commands:
        LOG.warning("[TOOL-LOOP] command limit reached")
        break

      command_timeout = min(self.tool_timeout, remaining())
      LOG.info("[TOOL-LOOP] exec command=%d iter=%d: %s", executed + 1, i, c[:160])
      rc, out = _exec_tool_command(c, self.work_dir, command_timeout)
      executed += 1
      feedback = f"[RESULT]\n{c}\n→ (rc={rc})\n{out}\n[/RESULT]"
      if rc == 0:
        # A successful payload becomes idempotently blocked on the next pass.
        seen[key] = AGENT_REPEAT_LIMIT
        feedback += (
          "\n[PROTOCOL]\nКоманда успешно выполнена ровно один раз. "
          "Не повторяй её и не создавай файл заново. Если задача завершена, "
          "ответь DONE: <краткое резюме с путём и проверкой>.\n[/PROTOCOL]"
        )
      if rc != 0:
        # Permit one retry after a failed command; the model may need to fix it.
        seen[key] = 0
      try:
        answer = ask_feedback(feedback)
      except Exception as exc:
        LOG.warning("[TOOL-LOOP] command feedback failed: %s", exc)
        break

    if _has_done_marker(answer):
      complete = True
    if not complete and self._extract_tool_commands(answer):
      LOG.warning("[TOOL-LOOP] unfinished tool output after limits")
    return {"model": self.selected_model, "answer": answer, "complete": complete,
            "tool_commands": executed}
...
@app.post("/agent/task")
def agent_task(req: AgentTaskRequest) -> dict[str, Any]:
  if worker is None:
    raise HTTPException(503, "worker не запущен")
  try:
    return worker.submit("agent_task", req.model_dump(), req.timeout + 30)
  except PermissionError as exc:
    raise HTTPException(401, str(exc), headers={"WWW-Authenticate": "Bearer"}) from exc
  except Exception as exc:
    raise HTTPException(502, str(exc)) from exc
...
def fs_write(req: FsWriteRequest) -> dict[str, Any]:
  ...
  try:
    ...
    return {"ok": True, "path": str(p), "bytes": len(req.content.encode("utf-8")), "append": req.append}
  except Exception as exc:
    raise HTTPException(502, str(exc)) from exc
...
def completions(req: ChatRequest):
  ...
  if req.stream:
    def stream_response():
      ...
      model_name = req.model or worker.selected_model
      for phrase in ("получил задание", "передаю его модели", "проверяю результат"):
        data = {"id": completion_id, "object": "chat.completion.chunk", "created": created,
                "model": model_name,
                "choices": [{"index": 0, "delta": {"thinking": phrase + " ", "reasoning_content": phrase + " "}, "finish_reason": None}]}
      ...
      try:
        ...
        if not complete:
          delta["x_arena"] = {"complete": False}
        data = {"id": completion_id, "object": "chat.completion.chunk", "created": created,
                "model": result.get("model", model_name),
                "choices": [{"index": 0, "delta": delta, "finish_reason": "stop" if complete else "length"}]}
        yield "data: " + json.dumps(data, ensure_ascii=False) + "\\n\\n"
        yield "data: [DONE]\\n\\n"
      except Exception as exc:
        data = {"error": {"message": str(exc), "type": type(exc).__name__}}
        yield "data: " + json.dumps(data, ensure_ascii=False) + "\\n\\n"
        yield "data: [DONE]\\n\\n"
  ...
  if not complete:
    response["x_arena"] = {"complete": False, "tool_commands": result.get("tool_commands", 0)}
  return response
...
def main() -> None:
  ...
  parser.add_argument("--task-root", default="/home/egor", help="Корень директорий, доступных для /agent/task")
  ...
  parser.add_argument("--max-tool-commands", type=int, default=AGENT_MAX_TOOL_COMMANDS, help="Макс. число реально выполняемых команд на одну задачу")
  ...
  worker = ArenaWorker(
    ...
    max_tool_commands=args.max_tool_commands,
