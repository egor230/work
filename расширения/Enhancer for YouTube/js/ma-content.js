...
(function () {
  ...
  function boot() {
    ...
    // Порядок приоритета: sessionStorage (текущая сессия) > storage.local (maTheme) > cookie PREF (f6) > авто.
    var quick = null;
    ...
    chrome.storage.local.get([STORAGE_KEY], function (result) {
      ...
      if (stored) {
        ...
        // зафиксировали выбранную тему в storage (впервые)
        chrome.storage.local.set({ maTheme: quick || getThemeFromCookie() });
      }
      startGuards();
    });
    createToggle();
