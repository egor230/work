...
class GamepadWidget(QWidget):
    ...
    zones_changed = pyqtSignal()  # Сигнал при изменении позиции/размера зон
    ...
    def __init__(self, image_path, parent=None):
        ...
        self._edit_enabled = False          # Флаг разрешения редактирования (перетаскивание/изменение размера)
        # Переменные для режима редактирования зон
        self._edit_mode = False              # Режим перетаскивания
        self._drag_zone = None               # Зона которую тащим
        self._drag_start_pos = None          # Начальная позиция мыши при перетаскивании
        self._drag_start_rx = None           # Начальные координаты зоны
    ...
    def set_debug_mode(self, on):
        """Включает/выключает режим редактирования (перетаскивание и изменение размеров зон)."""
    ...
    def paintEvent(self, event):
        ...
        for z in self.zones:
            ...
            # ВСЕГДА показываем подписи и полупрозрачный фон для назначенных кнопок
            if z.current_binding:
                ...
                # Для неназначенных кнопок показываем только имя зоны
                painter.setPen(QColor(255, 255, 255, 30))
                painter.setFont(QFont("Consolas", 7))
                painter.drawText(rect, Qt.AlignmentFlag.AlignCenter, z.name)

            # Эффект при наведении курсора — зажигает синим цветом
            ...
            # В режиме редактирования показываем рамку вокруг перетаскиваемой зоны
    ...
    def _zone_at(self, pos):
        for z in reversed(self.zones):
            if z.contains(pos, self.width(), self.height()):
                return z
        return None
    ...
    def mouseMoveEvent(self, event):
        ...
        if self._edit_mode:
            self.setCursor(Qt.CursorShape.OpenHandCursor if zone else Qt.CursorShape.ArrowCursor)
        else:
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            if zone:
                if self._edit_enabled:
                    # В режиме редактирования — начинаем перетаскивание
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            if self._edit_mode and self._drag_zone:
                ...
                self.zones_changed.emit()  # Сигнализируем об изменении зон
                self.update()
            else:
                self._press_timer.start()

    def wheelEvent(self, event):
        # Колёсико мыши меняет размер зоны под курсором (только в режиме редактирования)
        if self._edit_enabled:
            pos = event.position()
            zone = self._zone_at(pos)
            if zone:
                ...
                new_rw = max(0.01, min(0.5, zone.rw * scale))
                new_rh = max(0.01, min(0.5, zone.rh * scale))
                zone.rw = new_rw
                zone.rh = new_rh
...
class MainWindow(QWidget):
    ...
    def _build_footer(self):
        ...
        self.debug_cb = QCheckBox("Редактирование зон (F3)")
        self.debug_cb.setChecked(False)   # по умолчанию выключено (подписи видны, двигать нельзя)
