...
import asyncio, argparse, re, time, logging, random, aiohttp
...
def sanitize_channel_name(ch: Channel) -> Channel:
  ...
  if "svg" in low_name or "<" in low_name or ch.is_garbage_name:
    _bad_name_counter += 1
    ch.name = f"{_bad_name_counter}"
    ch.logo = ""
  return ch

def sort_channels(channels: List[Channel]) -> List[Channel]:
  def sort_key(ch: Channel):
    name_low = ch.clean_name.lower()
    
    match_index = -1
    for idx, pref in enumerate(PRIORITY_CHANNELS):
      if name_low.startswith(pref):
        match_index = idx
        break
        
    if match_index != -1 and ch.timeshift == 2:
      return (0, match_index, -ch.quality_score, name_low)
    ...
    return (1, ts_rank, country_rank, name_rank, quality, alpha)
...
async def main():
  ...
  log.info(f"ГОТОВО за {minutes}:{seconds:02d} мин:сек")
