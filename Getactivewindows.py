...
import time, json, os, copy, psutil, subprocess, pyautogui, re, threading
  ...
  # Выводим результаты
   ...
   for line in result:   # print(line)
     ...
     if user_name==user and pid_id==process_id_active:# Только процессоры запущенны от имени пользователя
      ...
      if '.exe' in filename:
       print("filename ", filename, " dir = ", dir_process_name, sep=" ")
       # a.append({'dir': dir_process_name, 'pid': pid_id, 'exe': filename})# нашли pid активного  окна
...
while 1:
  time.sleep(3)
  check_current_active_window(user)